...
addon_name = 'Watchdogs'
addon_icon = xbmcaddon.Addon().getAddonInfo('icon')
addon_path = xbmc.translatePath(('special://home/addons/plugin.video.watchdogs')).decode('utf-8')
module_path = xbmc.translatePath(('special://home/addons/script.module.watchdogs')).decode('utf-8')

control.execute('RunPlugin(plugin://%s)' % control.get_plugin_url({'action': 'service'}))

def syncTraktLibrary():
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.watchdogs/?action=tvshowsToLibrarySilent&url=traktcollection')
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.watchdogs/?action=moviesToLibrarySilent&url=traktcollection')

try:
    ModuleVersion = control.addon('script.module.watchdogs').getAddonInfo('version')
    AddonVersion = control.addon('plugin.video.watchdogs').getAddonInfo('version')
    RepoVersion = control.addon('repository.watchdogs').getAddonInfo('version')
    ...
    log_utils.log('### Watchdogs SCRIPT VERSION: %s ###' % str(ModuleVersion), log_utils.LOGNOTICE)
    log_utils.log('### Watchdogs REPOSITORY VERSION: %s ###' % str(RepoVersion), log_utils.LOGNOTICE)
    log_utils.log('###############################################################', log_utils.LOGNOTICE)
except:
    log_utils.log('######################### Watchdogs ############################', log_utils.LOGNOTICE)
    log_utils.log('####### CURRENT Watchdogs VERSIONS REPORT ######################', log_utils.LOGNOTICE)
    log_utils.log('### ERROR GETTING Watchdogs VERSIONS - NO HELP WILL BE GIVEN AS THIS IS NOT AN OFFICIAL Watchdogs INSTALL. ###', log_utils.LOGNOTICE)
    log_utils.log('###############################################################', log_utils.LOGNOTICE)
...
def main():
    fum_ver = xbmcaddon.Addon(id='script.module.watchdogs').getAddonInfo('version')
    updated = xbmcaddon.Addon(id='plugin.video.watchdogs').getSetting('module_base')
    ...
    log('watchdogs Scraper Path: %s' % (str(scraper_path)), LOGINFO)
    try:
        xml = openfile(settings_xml_path)
    except Exception:
        failure = traceback.format_exc()
        log('watchdogs Service - Exception: \n %s' % (str(failure)), LOGINFO)
        return
    ...
    xbmcaddon.Addon(id='plugin.video.watchdogs').setSetting('module_base', fum_ver)
...
DEBUGPREFIX = '[COLOR red][ Watchdogs DEBUG ][/COLOR]'
