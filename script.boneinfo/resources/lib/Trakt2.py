...
def handle_trakt_movies(results):
    ...
    for movie in results:
        if SETTING("infodialog_onclick") != "false":
            path = 'plugin://script.boneinfo/?info=boneinfo&id=%s' % str(fetch(movie["movie"]["ids"], 'tmdb'))
        else:
            path = "plugin://script.boneinfo/?info=playtrailer&id=" + str(fetch(movie["movie"]["ids"], 'tmdb'))
...
def handle_trakt_tvshows(results):
    ...
    for tvshow in results:
        ...
        path = 'plugin://script.boneinfo/?info=extendedtvinfo&tvdb_id=%s' % tvshow['show']['ids']["tvdb"]
