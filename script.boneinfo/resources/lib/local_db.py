...
def handle_db_movies(movie):
    trailer = "plugin://script.boneinfo/?info=playtrailer&dbid=%s" % str(movie['movieid'])
    if SETTING("infodialog_onclick") != "false":
        path = 'plugin://script.boneinfo/?info=boneinfo&dbid=%s' % str(movie['movieid'])
    else:
        path = 'plugin://script.boneinfo/?info=playmovie&dbid=%i' % movie['movieid']
...
def handle_db_tvshows(tvshow):
    if SETTING("infodialog_onclick") != "false":
        path = 'plugin://script.boneinfo/?info=extendedtvinfo&dbid=%s' % str(tvshow['tvshowid'])
    else:
        path = 'plugin://script.boneinfo/?info=action&id=ActivateWindow(videos,videodb://tvshows/titles/%s/,return)' % str(tvshow['tvshowid'])
...
def compare_album_with_library(online_list):
    ...
    for online_item in online_list:
        for local_item in local_list:
            ...
            online_item["path"] = 'plugin://script.boneinfo/?info=playalbum&dbid=%i' % album['albumid']
            if album["thumbnail"]:
