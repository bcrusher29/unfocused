...
class DialogBaseInfo(object):
    ...
    def onInit(self, *args, **kwargs):
        ...
        HOME.setProperty("boneinfo_fanart", self.info.get("fanart", ""))
            ...
            if selection == 0:
                if self.type == "TVShow":
                    if dbid:
                        ...
                        xbmc.executebuiltin("ActivateWindow(10025,plugin://plugin.video.boneplayer/tv/tvdb/%s,return)" % tvdb)
                        self.close()
                else:
                    if dbid: url = "temp"
                    else:
                        dbid = 0
                        url = "plugin://plugin.video.boneplayer/movies/play/tmdb/%s/%s" % (item_id, SETTING("player_main_movie"))
                    PLAYER.qlickplay(url, listitem=None, window=self, dbid=dbid)
            if selection == 1:
                if self.type == "TVShow":
                    if dbid:
                        show_path = xbmc.translatePath("special://profile/addon_data/plugin.video.boneplayer/TVShows/%s" % tvdb)
                        if os.path.exists(show_path):
                            try:
                                shutil.rmtree(show_path)
                                xbmc.executebuiltin("RunScript(script.boneinfo,info=afteradd)")
                    ...
                    if dbid:
                        movie_path = xbmc.translatePath("special://profile/addon_data/plugin.video.boneplayer/Movies/%s" % imdb)
