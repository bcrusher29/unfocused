...
def get_movie_window(window_type):
    ...
    class DialogVideoInfo(DialogBaseInfo, window_type):
        ...
        def add_list_to_account(self):
            ...
            if selection == -1:
                ...
                for item in movies_list:
                    if item != "":
                        ids = ids + str(item[1]) + '\n'
                batch_add_file_path = "special://profile/addon_data/plugin.video.boneplayer/movies_to_add.txt"
                ...
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.boneplayer/movies/batch_add_to_library)")
                xbmc.executebuiltin("Dialog.Close(busydialog)")
            elif selection > 0:
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.boneplayer/movies/add_to_library/tmdb/%s)" % str(movies_list[selection][1]))
        ...
        @ch.click(120)
        def search_in_boneplayer_by_title(self):
            url = "plugin://plugin.video.boneplayer/movies/tmdb/search_term/%s/1" % self.info.get("title", "")
            self.close()
        ...
        def trakt_manager_dialog(self):
            ...
            content = "movie"
            xbmc.executebuiltin("RunScript(script.boneinfo,info=traktManager,name=%s,tmdb=%s,content=%s)" % (name, item_id, content))
        ...
        def play_movie_no_resume(self):
            if self.dbid:
                ...
                dbid = 0
                url = "plugin://plugin.video.boneplayer/movies/play/tmdb/%s/%s" % (self.info.get("id", ""), SETTING("player_main_movie"))
        ...
        def show_manage_dialog(self):
            ...
            manage_list.append([xbmc.getInfoLabel('System.AddonTitle(%s)' % 'plugin.video.boneplayer') + " " + LANG(10004), 'Addon.OpenSettings(%s)' % 'plugin.video.boneplayer'])
        ...
        def appintegration_movie(self):
            ...
            if xbmc.getCondVisibility("system.hasaddon(script.boneinfo)"):
                manage_list.append(["Search Youtube Videos", "RunScript(script.boneinfo,info=list,type=video,query="+title+")||Notification(YouTube Videos:,"+title+",5000,special://home/addons/script.boneinfo/resources/skins/Default/media/common/youtube.png)"])
            if xbmc.getCondVisibility("system.hasaddon(script.boneinfo)") and ADDON.getSetting("ExtendedYoutubeMovie") == "true":
                manage_list.append(["Search Youtube Channels", "RunScript(script.boneinfo,info=list,type=channel,query="+title+")||Notification(YouTube Channels:,"+title+",5000,special://home/addons/script.boneinfo/resources/skins/Default/media/common/youtube.png)"])
            if xbmc.getCondVisibility("system.hasaddon(script.boneinfo)") and ADDON.getSetting("ExtendedYoutubeMovie") == "true":
                manage_list.append(["Search Youtube Playlists", "RunScript(script.boneinfo,info=list,type=playlist,query="+title+")||Notification(YouTube Playlists:,"+title+",5000,special://home/addons/script.boneinfo/resources/skins/Default/media/common/youtube.png)"])
        ...
        def pick_and_mix(self):
            dbid = 0
            url = "plugin://plugin.video.boneplayer/movies/play/tmdb/%s/%s" % (self.info.get("id", ""), self.listitem.getProperty("qid"))
        ...
        def add_movie_to_library(self):
            MovieLibrary = boneplayer.getSetting("movies_library_folder")
            imdb_id = self.info["imdb_id"]
            if not os.path.exists(xbmc.translatePath("%s%s/" % (MovieLibrary, imdb_id))):
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.boneplayer/movies/add_to_library/tmdb/%s)" % self.info.get("id", ""))
