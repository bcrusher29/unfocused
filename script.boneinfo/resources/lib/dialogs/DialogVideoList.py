    ...
    class DialogVideoList(DialogBaseList, window_type):
        ...
        def context_menu(self):
            ...
            if selection != -1 and listitems[selection] == LANG(32255):
                ...
                if self.type == "tv" or self.type == "episode":
                    if self.listitem.getProperty("dbid"):
                        ...
                        if os.path.exists(show_playlist):
                            url = show_playlist
                        else: url = 'plugin://plugin.video.boneplayer/tv/play/%s/1/1/library' % (tvdb_id)
                    else: url = 'plugin://plugin.video.boneplayer/tv/play/%s/1/1/library' % (tvdb_id)
                else:
                    if self.listitem.getProperty("dbid"): url = 'temp'
                    else: url = 'plugin://plugin.video.boneplayer/movies/play/tmdb/%s/library' % (item_id)
                PLAYER.qlickplay(url, listitem=None, dbid=dbid, window=self)
            elif selection == 1:
                if self.type == "tv" or self.type == "episode":
                    TVLibrary   = boneplayer4QED.getSetting("tv_library_folder")
                    TVPlaylists = boneplayer4QED.getSetting("tv_playlist_folder")
                    if self.listitem.getProperty("dbid"):
                        ...
                        xbmc.executebuiltin("RunPlugin(plugin://plugin.video.boneplayer/tv/add_to_library/%s)" % tvdb_id)
                    ...
                    MovieLibrary = boneplayer4QED.getSetting("movies_library_folder")
                    if self.listitem.getProperty("dbid"):
                        ...
                        xbmc.executebuiltin("RunPlugin(plugin://plugin.video.boneplayer/movies/add_to_library/tmdb/%s)" % item_id)
                ...
                if self.type == "tv": url = "plugin://script.boneinfo/?info=playtvtrailer&id=%s" % item_id
                elif self.type == "episode": url = "plugin://script.boneinfo/?info=playtvtrailer&tvdb_id=%s" % tvdb_id
                elif self.type == "movie": url = "plugin://script.boneinfo/?info=playtrailer&id=%s" % item_id
                ...
                if self.logged_in:
                    ...
                    if self.type == "episode":
                        path = xbmc.translatePath("special://profile/addon_data/script.boneinfo/exclusions_tvdb.txt")
