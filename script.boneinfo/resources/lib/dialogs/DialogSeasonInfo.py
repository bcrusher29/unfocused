    ...
    class DialogSeasonInfo(DialogBaseInfo, window_type):
        ...
        def browse_season(self):
            if self.dbid:
                ...
                url = "plugin://plugin.video.boneplayer/tv/tvdb/%s/%s/" % (tvdb_id, self.info.get("season", ""))
            self.close()
        ...
        def play_season_no_resume(self):
            if self.dbid:
                ...
                if not os.path.exists(url):
                    url = "plugin://plugin.video.boneplayer/tv/play/%s/%s/1/%s" % (self.info.get("tvdb_id", ""), self.info.get("season", ""), SETTING("player_main_tv"))
            else:
                url = "plugin://plugin.video.boneplayer/tv/play/%s/%s/1/%s" % (self.info.get("tvdb_id", ""), self.info.get("season", ""), SETTING("player_main_tv"))
        ...
        def pick_and_mix(self):
            url = 'plugin://plugin.video.boneplayer/tv/play/%s/%s/1/%s' % (self.info.get("tvdb_id", ""), self.info.get("season", ""), self.listitem.getProperty("qid"))
        ...
        def change_kodi_fav_status(self):
            ...
            get_kodi_json(method="Favourites.AddFavourite", params='{"title":"%s", "type":"window", "window":"Videos", "windowparameter":"%s", "thumbnail":"%s"}' % (fav_name, 'plugin://script.boneinfo/?info=seasoninfo&id=%s&tvshow=%s&season=%s' % (self.tvshow_id, self.info["TVShowTitle"], self.info["season"]), self.info["poster"]))
