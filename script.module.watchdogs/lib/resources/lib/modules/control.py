...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.watchdogs.artwork)'):
        return os.path.join(
            xbmcaddon.Addon('script.watchdogs.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.watchdogs.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.watchdogs.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.watchdogs.metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.watchdogs.metadata').getAddonInfo('path'),
