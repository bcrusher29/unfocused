...
import urlparse
from crucialmindsscrapers.modules import control
from crucialmindsscrapers import providerSources, providerNames

params = dict(urlparse.parse_qsl(sys.argv[2].replace('?', '')))
mode = params.get('mode')

def ScraperChoice():
    from crucialmindsscrapers import providerSources
...
def ToggleProviderAll(enable):
    from crucialmindsscrapers import providerNames
...
def toggleAll(setting, open_id=None, sourceList=None):
    from crucialmindsscrapers import getAllHosters
...
if mode == "CrucialmindsSettings":
    control.openSettings('0.0', 'script.module.crucialmindsscrapers')
