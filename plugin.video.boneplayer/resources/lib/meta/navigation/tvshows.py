...
def tv_add_all_to_library(items, noscan = False):
    ...
    batch_add_file.close()
    xbmc.executebuiltin("RunPlugin(plugin://plugin.video.boneplayer/tv/batch_add_to_library)")
        ...
        for id in ids:
            ...
            if id == None or id == "None": pass
                ...
                try:
                    ...
                    xbmc.log("boneplayer failed to add: {0}".format(id),xbmc.LOGNOTICE)
            ids_index += 1
...
def make_tvshow_item(info):
    ...
    elif xbmc.getCondVisibility("system.hasaddon(script.boneinfo)"): context_menu = [(_("Scan item to library"),"RunPlugin({0})".format(plugin.url_for("tv_add_to_library", id=tvdb_id))), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.boneinfo,info=playtvtrailer,tvdb_id={0})".format(tvdb_id)), (_("Extended TV show info"), "RunScript(script.boneinfo,info=extendedtvinfo,tvdb_id={0})".format(tvdb_id)), ("%s %s (%s)" % ("Recommended", _("TV shows"), "TMDb"),"ActivateWindow(10025,plugin://script.boneinfo/?info=similartvshows&tvdb_id={0})".format(tvdb_id))]
...
def list_seasons_tvdb(id):
    ...
    for (season_num, season) in show.items():
        ...
        elif xbmc.getCondVisibility("system.hasaddon(script.boneinfo)"): context_menu = [(_("Extended season info"), "RunScript(script.boneinfo,info=seasoninfo,tvshow={0},season={1})".format(title, season_num)), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.boneinfo,info=playtvtrailer,tvdb_id={0})".format(id)), (_("Recommended tv shows") + " (TMDb)","ActivateWindow(10025,plugin://script.boneinfo/?info=similartvshows&tvdb_id={0})".format(id))]
        else: context_menu = []
...
def list_episodes_tvdb(id, season_num):
    ...
    for (episode_num, episode) in season.items():
        ...
        elif xbmc.getCondVisibility("system.hasaddon(script.boneinfo)"): context_menu = [(_("Extended episode info"), "RunScript(script.boneinfo,info=episodeinfo,tvshow={0},season={1},episode={2})".format(title, season_num, episode_num)), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.boneinfo,info=playtvtrailer,tvdb_id={0})".format(id)), (_("Recommended tv shows") + " (TMDb)","ActivateWindow(10025,plugin://script.boneinfo/?info=similartvshows&tvdb_id={0})".format(id))]
        else: context_menu = []
