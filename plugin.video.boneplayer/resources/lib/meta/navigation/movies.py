...
def movies_batch_add_to_library():
    ...
    if xbmcvfs.exists(movie_batch_file):
        ...
        if xbmcvfs.exists(plugin.get_setting(SETTING_TV_BATCH_ADD_FILE_PATH, unicode)):
            ...
            scan_library(type="video", path=plugin.get_setting(SETTING_MOVIES_LIBRARY_FOLDER, unicode))
            return True
    if xbmcvfs.exists(plugin.get_setting(SETTING_TV_BATCH_ADD_FILE_PATH, unicode)): xbmc.executebuiltin("RunPlugin(plugin://plugin.video.boneplayer/tv/batch_add_to_library)")
...
def make_movie_item(movie_info, is_list = False):
    ...
    elif xbmc.getCondVisibility("system.hasaddon(script.boneinfo)"): context_menu = [(_("Scan item to library"),"RunPlugin({0})".format(plugin.url_for("movies_add_to_library", src=src, id=id))), ("%s %s" % (_("Movie"), _("Trailer").lower()),"RunScript(script.boneinfo,info=playtrailer,id={0})".format(id)), (_("Extended movie info"), "RunScript(script.boneinfo,info=boneinfo,id={0})".format(id)), ("%s %s (%s)" % ("Recommended", _("movies"), "TMDb"),"ActivateWindow(10025,plugin://script.boneinfo/?info=similarmovies&id={0})".format(id))]
