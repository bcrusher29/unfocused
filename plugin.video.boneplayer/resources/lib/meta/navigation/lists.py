...
def write_list_id_files(misc_ids, tv_ids, movie_ids, slug, user):
    if len(misc_ids) > 0:
        misc_list_file_path = "special://profile/addon_data/plugin.video.boneplayer/misc_from_" + slug + "_by_" + user + ".txt"
    ...
    if len(tv_ids) > 0:
        shows_list_file_path = "special://profile/addon_data/plugin.video.boneplayer/shows_from_" + slug + "_by_" + user + ".txt"
        ...
        if not xbmcvfs.exists(shows_list_file_path):
            ...
            batch_add_file.close()
    if len(movie_ids) > 0:
        movies_list_file_path = "special://profile/addon_data/plugin.video.boneplayer/movies_from_" + slug + "_by_" + user + ".txt"
...
def write_batch_id_files(tv_ids, movie_ids, misc_ids):
    if len(tv_ids) > 0:
        shows_import_file_path = "special://profile/addon_data/plugin.video.boneplayer/shows_to_add.txt"
    ...
    if len(movie_ids) > 0:
        movies_import_file_path = "special://profile/addon_data/plugin.video.boneplayer/movies_to_add.txt"
    ...
    if len(misc_ids) > 0:
        misc_import_file_path = "special://profile/addon_data/plugin.video.boneplayer/misc_to_add.txt"
...
def lists_trakt_add_liked_to_library():
    ...
    pDialog.create('[COLOR yellow]BoneCrusher[/COLOR]', 'Creating batch import files')
    ...
    for list_item in list_items:
        ...
        if item_type == "show":
            ...
            if src == "": item = None
                ...
                elif xbmc.getCondVisibility("system.hasaddon(script.boneinfo)"):
                    context_menu = [( _("Scan item to library"), "RunPlugin({0})".format(plugin.url_for("movies_add_to_library", src=src, id=id))),
                                    ("%s %s" % (_("Movie"), _("Trailer").lower()), "RunScript(script.boneinfo,info=playtrailer,id={0})".format(id)), (
                                    (_("Extended movie info"),_("Extended movie info"), "RunScript(script.boneinfo,info=boneinfo,id={0})".format(id)), 
                                    ("%s %s (%s)" % ("Recommended", _("movies"), "TMDb"), "ActivateWindow(10025,plugin://script.boneinfo/?info=similarmovies&id={0})".format(id)))]
                else:
                    context_menu = [( _("Scan item to library"), "RunPlugin({0})".format(plugin.url_for("movies_add_to_library", src=src, id=id)))]
